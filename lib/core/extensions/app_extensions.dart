...
extension PaddingExt on Widget {
  ...
  Padding pAll(double value, {Key? key}) => Padding(key: key, padding: EdgeInsets.all(value), child: this);

  Padding pOnly({double left = 0.0, double top = 0.0, double right = 0.0, double bottom = 0.0, Key? key}) => Padding(
    key: key,
...
extension SizedBoxExtension on Widget {
  Widget w(double width, {Key? key}) => SizedBox(key: key, width: width, child: this);

  Widget wFull(BuildContext context, {Key? key}) => SizedBox(key: key, width: context.width, child: this);

  Widget h(double? height) => SizedBox(key: key, height: height, child: this);

  Widget hFull(BuildContext context, {Key? key}) => SizedBox(key: key, height: context.height, child: this);

  Widget wh(double? width, double? height) => SizedBox(key: key, width: width, height: height, child: this);

  Widget whFull(BuildContext context, {Key? key}) =>
      SizedBox(key: key, width: context.width, height: context.height, child: this);
}
...
extension PositionedExtension on Widget {
  Widget positioned({double? top, double? bottom, double? left, double? right}) =>
